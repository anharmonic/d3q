...
PROGRAM q2r
 ...
 integer nRlist,iper,nqacc,qacc
  ...
  complex(DP),allocatable :: mat(:,:,:,:,:,:,:)!(3,3,3,nax,nax,nax,nRlistx)
  ...
  NAMELIST / input / nr1, nr2, nr3, fileout, nqptsx
...
! ALLOCATE(q1c(3,nrtot),q2c(3,nrtot))
 ...
 ALLOCATE (phiqo(3*nax,3*nax,3*nax,nqx))
...
if (nat.ne.nax) call errore('reading ',' nax .ne. nat',1)
 
 ALLOCATE(phiqn2(3*nax,3*nax,3*nax,nqx))
 !
 nqs=nqpts 
 write(*,*) ' calling qaccepted'
 call qaccepted(nqacc,nqx,nax,nat,nqs,at,bg,q1o,q2o,q3o,phiqo,q1n2,q2n2,q3n2,phiqn2)
 write(*,*)'nqacc', nqacc
 !
 ! :lp: cleanup
 deallocate(q1o,q2o,q3o)
 deallocate(phiqo)
 ALLOCATE(phiqn(3*nax,3*nax,3*nax,nqx))
                     ...
                     do ic = 1, 3
                        ...
                        do jc = 1, 3
                           ...
                           do kc = 1, 3
                              ...
                              aus(ic,jc,kc) = aus(ic,jc,kc) + phiqn(ii,jj,kk,iq) * phase * norm
                           enddo
                        enddo
                     enddo
                  end do
         ...
         do na3=1,nat 
            ...
            do iq= 1,nqacc
               ...
               do iRlist=1,nRlist
                  ...
                  aus1 = aus1 + cmplx(cos(arg),sin(arg),kind=DP)*mat(:,:,:,na1,na2,na3,iRlist)
...
ALLOCATE (phiq2(3*nax,3*nax,3*nax,nqx))
maxdiffq=0.0d0
do iq=1,nqacc
   do na3=1,nat
      do kcar=1,3
...
110  continue
     ...
     ntmp = nqpts
     CALL permutations (ntmp,nqptsx,nat,nat,nqpts,at,bg, xq1, xq2, xq3, matq)
...
!---------------------------------------------
!subroutine  listr(rout,rrbig,iout,nout)
  
!use common_var, only : DP
!implicit none
!  ! I/O variables
!  integer :: nout,iout
!  integer, parameter:: noutx=1000000
!  real(DP) :: rout(3,noutx),rrbig(3),ddist,eps
!  ! local variables
!  integer :: i, count
!  eps=1.0d-6
...
!  count=0
!  do i=1,nout
!     ddist= dsqrt((rrbig(1)-rout(1,i))**2.0d0 +(rrbig(2)-rout(2,i))**2.0d0 +(rrbig(3)-rout(3,i))**2.0d0 ) 

!     if (ddist.le.eps) then
!        iout=i
!     else
!!      write(*,*) ddist,rrbig(1),rout(1,i)
!        count=count+1
!     end if
!  end do
...
!  return
!end subroutine listr

!-----------------------------------------------------------------------------------------------------
!-----------------------------------------------------------------------------------------------------

subroutine  permutations (nnc,nqx,nax,nat,nqs,at,bg, q1n,q2n,q3n,phiq)
  !
...
subroutine  qaccepted(nqacc,nqx,nax,nat,nqs,at,bg,q1o,q2o,q3o,phiqo,q1n,q2n,q3n,phiqn)
  !
...
implicit none
  ! I/O variables
  integer :: nqx, nax, nat, nqs,nqacc,nequiv
  !parameter (nqsoutx=100000)
  real(DP) :: q1n(3,nqx),q2n(3,nqx),q3n(3,nqx), at(3,3),bg(3,3)
  real(DP) :: q1o(3,nqx),q2o(3,nqx),q3o(3,nqx)
 complex(DP) :: phiqo(3*nax,3*nax,3*nax,nqx),phiqn(3*nax,3*nax,3*nax,nqx)
