...
!/nope/!$OMP END PARALLEL DO
        !
        dyn(:,:,it) = dyn(:,:,it) + sum_dynbubble( S%nat3, T(it), freq, bose, D3, nu0 )
        !
      ENDDO
      !
    ENDDO
    !
    dynbubble_q = -0.5_dp * dyn/grid%nq
    !
    DEALLOCATE(U)
    !
  ...
  FUNCTION sum_dynbubble(nat3, T, freq, bose, V3, nu0)
    ...
    sqfreqm1 = 0._dp
    DO i = 1,nat3
      IF(freq(i,1)/=0._dp) sqfreqm1(i) = DSQRT(0.5_dp/freq(i,1))
      IF(freq(i,2)/=0._dp) freqm1(i,2) = 0.5_dp/freq(i,2)
      IF(freq(i,3)/=0._dp) freqm1(i,3) = 0.5_dp/freq(i,3)
    ENDDO
    !
    dyn(:,:) = (0._dp, 0._dp)
    !
