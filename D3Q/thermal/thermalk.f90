...
!--------------------------------------------------------------------------------
SUBROUTINE setupOLD(isw)
  !--------------------------------------------------------------------------------
  USE constants, ONLY: DP, tpi, cone, czero, eps8
  USE common_variables, ONLY : &
    tsetup3, T1, T0, tdindiag, tphase, td3mm, &
    freq1, freq1_, freq2, freq2_, freq3b, freq3_, &
    zz1n, zz1_x, zz2n, zz2_x, zz2b_c_x, zz3_x, zz3b_x, zz3bn, &
    q1d, q1ws, q2d, q2ws, q3bws, q3ws, &
    phasem3_q2c, phasem3_q2, phasem3_q2_, phasem3_q1t, phasem3_q2_init, &
    ic_inn, ic_out, ic_med, &
    d3mmx_1, d3mmx, &
    ipq2_, iq1_, ipq3_, ipq3b_, &
    freq3b_, zz3n, freq3, &
    mat3_rzyB, mat3_rzB, mat3_rz, mat3_rr, mat3_r, mq2d, mat3q, mat3_rzy, &
    ispeed3, ispeed2, &
    const_kuk, const_k, &
    nRbig31, Rbig31, nRbig32, Rbig32, &
    nRbig31t, nat33, nat32, nat3, mat3qb, &
    on, neqq1, neqq2, neqq3, neqq3b
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     lklappq3 = .True.
     qsum(:) = q1ws(:, iq1_) + q2ws(:, ipq2_) + q3ws(:, ipq3_)
     IF(neqq1(iq1_) > 1 .Or. neqq2(ipq2_) > 1 .Or. neqq3(ipq3_) > 1) lklappq3 = .False.
     IF(qsum(1)**2+qsum(2)**2+qsum(3)**2 > 1.d-6) lklappq3 = .False.
     lklappq3b = .True.
     qsum(:) = q1ws(:, iq1_) - q2ws(:, ipq2_) + q3bws(:, ipq3b_)
     IF(neqq1(iq1_) > 1 .Or. neqq2(ipq2_) > 1 .Or. neqq3b(ipq3b_) > 1) lklappq3b = .False.
     IF(qsum(1)**2+qsum(2)**2+qsum(3)**2 > 1.d-6) lklappq3b = .False.
...
!     d3mm_1(:,:,:) =d3mmx_1(:,:,:,1)
!
     IF( .Not. lklappq3) d3mmx(:, :, :, :) = const_kuk * d3mmx(:, :, :, :)
     IF( .Not. lklappq3b) d3mmx_1(:, :, :, :) = const_kuk * d3mmx_1(:, :, :, :)
!
     IF(lklappq3) d3mmx(:, :, :, :) = const_k * d3mmx(:, :, :, :)
     IF(lklappq3b) d3mmx_1(:, :, :, :) = const_k * d3mmx_1(:, :, :, :)
!
!
     td3mm = td3mm + nanosec(T0) - T1
!
  END SELECT
END SUBROUTINE SETUPOLD
!--------------------------------------------------------------------------------
