...
!-----------------------------------------------------------------------
subroutine cryst_to_cart2 (nvec, vec, trmat, iflag)
  !-----------------------------------------------------------------------
  !
  ! Same as cryst_to_cart for a second-order tensor
  !
  use parameters
  implicit none
  !
  ...
  !          = bg ,  basis of the reciprocal-space lattice, for k-points
  ! if iflag=-1: the opposite
  real(kind=DP), intent(inout) :: vec (3, 3, nvec)
  ! coordinates of the vector (atomic positions or k-points) to be
  ! transformed - overwritten on output
  !
  !    local variables
  ...
  ! counter on vectors
  ! counter on polarizations
  real(kind=DP) :: vau1 (3,3), vau2(3,3)
  ! workspace
  ...
  do nv = 1, nvec
     if (iflag.eq.1) then
        ...
        do ii  = 1, 3
        do ii_ = 1, 3
           vau1 (ii, :) = vau1(ii,:) + trmat (ii, ii_) * vec (ii_,:,nv)
        enddo
        enddo
        vau2(:,:) = 0.d0
        do ii  = 1, 3
        do ii_ = 1, 3
           vau2 (:,ii) = vau2(:,ii) + trmat (ii, ii_) * vau1 (:,ii_)
        enddo
        enddo
     else
        vau1(:,:) = 0.d0
        do ii  = 1, 3
        do ii_ = 1, 3
           vau1 (ii,:) = vau1(ii,:) + vec (ii_,:,nv) * trmat (ii_, ii)
        enddo
        enddo
        vau2(:,:) = 0.d0
        do ii  = 1, 3
        do ii_ = 1, 3
           vau2 (:,ii) = vau2(:,ii) + vau1 (:,ii_) * trmat (ii_, ii)
        enddo
        enddo
     endif
     vec (:,:,nv) = vau2 (:,:)
...
!
!!----------------------------------------------------------------------
subroutine warning (routin, messag, ierr)
  !----------------------------------------------------------------------
  !
  use parameters
  implicit none
