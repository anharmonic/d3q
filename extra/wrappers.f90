...
MODULE wrappers
  ...
  PUBLIC  f_remove, rename, f_chdir, f_mkdir, f_rmdir, f_getcwd, f_get_vm_size
  ! more stuff:
  ...
  INTERFACE
    ...
    FUNCTION getcwd(buffer,size) BIND(C,name="getcwd") RESULT(r)
      ...
      TYPE(c_ptr)  :: r
    END FUNCTION
    FUNCTION get_vm_size() BIND(C,name="get_vm_size") RESULT(r)
      USE iso_c_binding
...
CONTAINS
  ...
  FUNCTION f_get_vm_size() RESULT(r)
    INTEGER :: r
    r= INT(DBLE(get_vm_size())/(1024**2)+.5_dp)
    IF(r<0) r = 0
  END FUNCTION
