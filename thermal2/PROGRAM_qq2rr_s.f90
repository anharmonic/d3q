...
PROGRAM qq2rr
  ...
  USE fc3_interpolate, ONLY : grid, sparse, fc3_grid_to_sparse
  ...
  IF (cmdline_param_logical('h')) THEN
      WRITE(*,*) "Syntax: ls anh*| d3_qq2rr.x NQX NQY NQZ [-o FILEOUT] [-f NFAR] [-t THRESH] [-w] [-s]"
      WRITE(*,*) ""
  ...
  !
  WRITE(*,*) "Sparse matrix..."
  CALL fc3_grid_to_sparse(S%nat, fc3, sfc3, thr)
  WRITE(*,*) "Sparse matrix done"
  !
  IF(filename /="none")THEN
    WRITE(*,*) "Writing sparse FCs to file..."
    CALL sfc3%write(filename, S)
  ENDIF
  !
